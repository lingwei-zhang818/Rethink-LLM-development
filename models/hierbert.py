...
class HierarchicalBert(nn.Module):
    ...
    def __init__(self, encoder, max_segments=64, max_segment_length=128, w1=1.0, w2=1.0):
        ...
        self.graph_seg_pos_embeddings = nn.Embedding(max_segments + 1, encoder.config.hidden_size,
                                               padding_idx=0,
                                               _weight=sinusoidal_init(max_segments + 1, encoder.config.hidden_size))
        ...
        self.graph_seg_encoder = nn.Transformer(d_model=encoder.config.hidden_size,
                                          nhead=encoder.config.num_attention_heads,
                                          batch_first=True, dim_feedforward=encoder.config.intermediate_size,
                                          activation=encoder.config.hidden_act,
                                          dropout=encoder.config.hidden_dropout_prob,
    ...
    def forward(self,
        ...
        graph_input_ids = input_ids[:,64:128,:]
        input_ids = input_ids[:, 0:64, :]

        graph_attention_mask = attention_mask[:, 64:128, :]
        attention_mask = attention_mask[:, 0:64, :]

        graph_token_type_ids = token_type_ids[:, 64:128, :]
        token_type_ids = token_type_ids[:, 0:64, :]
        ...
        encoder_outputs1 = self.encoder1(input_ids=input_ids_reshape,
        ...
        encoder_outputs1 = encoder_outputs1.contiguous().view(input_ids.size(0), self.max_segments,
                                                            self.max_segment_length,
                                                            self.hidden_size)

        # Gather CLS outputs per segment --> (4, 64, 768)
        encoder_outputs = encoder_outputs1[:, :, 0]



        graph_input_ids_reshape = graph_input_ids.contiguous().view(-1, graph_input_ids.size(-1))
        graph_attention_mask_reshape = graph_attention_mask.contiguous().view(-1, graph_attention_mask.size(-1))
        if graph_token_type_ids is not None:
            graph_token_type_ids_reshape = graph_token_type_ids.contiguous().view(-1, graph_token_type_ids.size(-1))
        else:
            graph_token_type_ids_reshape = None

        encoder_outputs2 = self.encoder2(input_ids=graph_input_ids_reshape,
                                         attention_mask=graph_attention_mask_reshape,
                                         token_type_ids=graph_token_type_ids_reshape)[0]

        # Reshape back to (batch_size, n_segments, max_segment_length, output_size) --> (4, 64, 128, 768)
        encoder_outputs2 = encoder_outputs2.contiguous().view(graph_input_ids.size(0), self.max_segments,
                                                              self.max_segment_length,
                                                              self.hidden_size)

        # Gather CLS outputs per segment --> (4, 64, 768)
        graph_encoder_outputs = encoder_outputs2[:, :, 0]

        #encoder_outputs = (encoder_outputs1*self.w1+encoder_outputs2*self.w2)/(self.w1+self.w2)
        ...
        graph_seg_mask = (torch.sum(graph_input_ids, 2) != 0).to(graph_input_ids.dtype)
        # Infer and collect segment positional embeddings
        graph_seg_positions = torch.arange(1, self.max_segments + 1).to(graph_input_ids.device) * graph_seg_mask
        # Add segment positional embeddings to segment inputs
        graph_encoder_outputs += self.seg_pos_embeddings(graph_seg_positions)

        # Encode segments with segment-wise transformer
        graph_seg_encoder_outputs = self.graph_seg_encoder(graph_encoder_outputs)

        seg_encoder_outputs = (self.w1*seg_encoder_outputs + self.w2*graph_seg_encoder_outputs)/(self.w1+self.w2)
