...
class ModelArguments:
    ...
    we1: float = field(
        default=1.0, metadata={"help": "Whether to use a hierarchical variant or not"}
    )
    we2: float = field(
        default=1.0, metadata={"help": "Whether to use a hierarchical variant or not"}
    )
...
def main():
    ...
    if training_args.do_train:
        train_dataset = pd.read_json('./data/Graph_ecthr_train.json')
        train_dataset = datasets.Dataset.from_pandas(train_dataset)

    if training_args.do_eval:
        eval_dataset = pd.read_json('./data/Graph_ecthr_val.json')
        eval_dataset = datasets.Dataset.from_pandas(eval_dataset)

    # print(eval_dataset)

    if training_args.do_predict:
        predict_dataset = pd.read_json('./data/Graph_ecthr_test.json')
    ...
    model_encoder = HierarchicalBert(encoder=segment_encoder, w1=model_args.we1, w2=model_args.we2, max_segments=64, max_segment_length=128)
    #model_encoder = HierarchicalBert(encoder=segment_encoder, max_segments=64, max_segment_length=128)
    model.bert = model_encoder

    # MODEL_PATH = '/cpfs/user/cht/ecthr/lex-glue/log_adj/ecthr_a/nlpaueb/legal-bert-base-uncased/seed1_500.0:1.0'
    ...
    def preprocess_function(examples):
        ...
        if model_args.hierarchical:
            ...
            if config.model_type == 'roberta':
                ...
                B = examples['graph_text']
                for i in range(len(examples['text'])):
                    text = A[i]
                    graph = B[i]
                    case_encodings = tokenizer(text, padding=padding,
                                               max_length=data_args.max_seg_length, truncation=True)
                    i = case_encodings['input_ids'] + case_template * (
                            data_args.max_segments - len(case_encodings['input_ids']))
                    a = case_encodings['attention_mask'] + case_template * (
                            data_args.max_segments - len(case_encodings['attention_mask']))
                    t = case_encodings['token_type_ids'] + case_template * (
                            data_args.max_segments - len(case_encodings['token_type_ids']))

                    case_encodings = tokenizer(graph, padding=padding,
                                               max_length=data_args.max_seg_length, truncation=True)
                    i = i + case_encodings['input_ids'] + case_template * (
                            data_args.max_segments - len(case_encodings['input_ids']))
                    a = a + case_encodings['attention_mask'] + case_template * (
                            data_args.max_segments - len(case_encodings['attention_mask']))
                    t = t + case_encodings['token_type_ids'] + case_template * (
                            data_args.max_segments - len(case_encodings['token_type_ids']))

                    batch['input_ids'].append(i)
                    batch['attention_mask'].append(a)
                    batch['token_type_ids'].append(t)
