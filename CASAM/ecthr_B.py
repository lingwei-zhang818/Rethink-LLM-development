...
class ModelArguments:
    ...
    weight: float = field(
        default=0.0, metadata={"help": "Whether to use a hierarchical variant or not"}
    )
...
def main():
    ...
    if training_args.do_train:
        train_dataset = pd.read_json('./data/Graph_ecthr_mask_train.json')
    ...
    if training_args.do_eval:
        eval_dataset = pd.read_json('./data/Graph_ecthr_mask_val.json')
    ...
    if training_args.do_predict:
        predict_dataset = pd.read_json('./data/Graph_ecthr_mask_test.json')
        # predict_dataset = predict_dataset[0:50]
        # predict_dataset = predict_dataset[0:10]
        predict_dataset = datasets.Dataset.from_pandas(predict_dataset)
    ...
    def preprocess_function(examples):
        ...
        if model_args.hierarchical:
            ...
            if config.model_type == 'roberta':
                ...
                for case in examples['text']:
                    ...
                    batch['input_ids'].append(case_encodings['input_ids'] + case_template * (
                            data_args.max_segments - len(case_encodings['input_ids'])))
                    tmp.append(case_encodings['attention_mask'] + case_template * (
                            data_args.max_segments - len(case_encodings['attention_mask'])))
        ...
        tmp = np.array(tmp)
        tmp1 = np.array(examples['mask'])
        tmp = tmp+tmp1*model_args.weight
        new_shape = tmp.shape
        # print(new_shape)
        att_mask = np.zeros((new_shape[0], new_shape[1], new_shape[2],new_shape[2]))
        for passage in range(new_shape[0]):
            for doc in range(new_shape[1]):
                for row in range(new_shape[2]):
                    for column in range(128):
                        if tmp[passage, doc, row] == 0:
                            att_mask[passage,doc, row, column] = 0
                            continue
                        if tmp[passage, doc, column] == 0:
                            att_mask[passage,doc, row, column] = 0
                            continue
                        if tmp[passage, doc, row] != 1 and tmp[passage, doc, column] != 1:
                            att_mask[passage,doc, row, column] = tmp[passage, doc, column]
                        else:
                            att_mask[passage,doc, row, column] = 1
            # print(att_mask[0,0,:,:])
            # exit()
        # for passage in range(new_shape[0]):
        #     for doc in range(new_shape[1]):
        #         for row in range(new_shape[2]):
        #             for column in range(128):
        #                 if att_mask[passage,doc, row, column] !=0 and att_mask[passage,doc, row, column] !=1:
        #                     flag = att_mask[passage,doc, row, column]
        #                     if tmp[passage, doc, column] != flag and tmp[passage, doc, row]!=flag:
        #                         print('No!\n')
        #                         exit()
        ...
        # exit()
        batch['attention_mask'] = tmp

        batch["labels"] = [[1 if label in labels else 0 for label in label_list] for labels in examples["judge_labels"]]
